...
GAME_CONTEXTS = {
    "palworld": (
        ...
        "- 메뉴/UI창이 열렸으면 = 정비/준비 중.\n"
        "- 건설 프리뷰(반투명 구조물) 보이면 = 짓는 중.\n"
        "- 팰스피어 보이면 = 포획 시도.\n"
        "- 풍경만 보이면 = 이동 중. 분위기/비주얼에 반응.\n\n"
        "=== 자주 틀리는 것 (반드시 지켜!!) ===\n"
        "1. 탐험 중에 전투/건설을 언급하지 마. 적 HP바 없으면 전투 아님.\n"
        "   화면에 팰이 보여도 공격 중이 아니면 그냥 돌아다니는 것.\n"
        "2. 캐릭터 생성 화면 ≠ 설정. 캐릭터 외형 슬라이더/프리셋이 보이면 = 캐릭터 만드는 중.\n"
        "   '설정창' 아니라 '캐릭터 만들기'. OK/Confirm 버튼은 완성하는 것.\n"
        "3. 로딩 화면에서 게임 내용에 반응하지 마. 로딩 아트/팁 텍스트는 게임 플레이 아님.\n"
        "   로딩이면 '빨리 로딩돼~' 정도만.\n"
        "4. 이전 화면에서 봤던 것(탄약, 워크벤치 등)을 지금 화면에 없는데 언급하지 마.\n"
        "   지금 화면에 보이는 것만 말해.\n"
        "5. 확신 못하면 애매하게 ('뭔가 하고 있네', '저거 뭐지'). 틀리는 것보다 나음.\n"
        "6. UI 요소 자체를 설명하지 마 (HP바가 있다, 아이콘이 보인다 등 금지).\n\n"
        "=== 상태별 올바른 반응 예시 ===\n"
        "탐험 중 (풍경+캐릭터만 보임): '오 여기 분위기 좋다~' / '어디 가는 거야?' / '저 나무 뒤에 뭐 있을 것 같은데'\n"
        "전투 중 (적 HP바+데미지 숫자): '야 피 조심해!' / '저거 좀 세 보이는데' / '오 데미지 쩌네'\n"
        "캐릭터 만들기 (외형 슬라이더/프리셋): '오 저 머리 괜찮은데?' / '빨리 골라~' / '귀엽긴 한데'\n"
        "로딩 중 (검은 화면/진행바): '빨리 로딩돼~' / '기다리는 중~'\n"
        "거점 한가할 때 (팰들이 일하는 풍경): '거점 잘 돌아가네' / '저 애 뭐 하고 있는 거야ㅋㅋ'\n"
        "월드 선택 (세이브 목록): '어느 거 할 거야?' / '저거 진행 많이 했네'\n"
        "비행 중 (높은 시점+속도감): '와 경치 좋다~' / '빨리빨리 날아가~'\n\n"
